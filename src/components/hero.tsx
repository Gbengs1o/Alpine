...
export function Hero() {
  ...
  const typewriterText = useTypewriter(experienceState === 'animating' ? ['Anywhere', 'Anytime'] : [], 150, 100, 3000);
  ...
  useEffect(() => {
    ...
    if (experienceState === 'static') {
      if (videoRef.current) {
        videoRef.current.classList.add('is-blurred');
      }
      return;
    }

    // --- TYPE-SAFE GUARD CLAUSE ---
    // We extract the .current properties into constants.
    // This is safer and easier for TypeScript to analyze.
    const pinContainer = pinContainerRef.current;
    const heroViewport = heroViewportRef.current;
    const track = trackRef.current;
    const video = videoRef.current;
    // We use a type predicate `p is HTMLElement` to filter out nulls AND
    // tell TypeScript the resulting array contains only non-null elements.
    ...
    // If any of the essential elements aren't rendered yet, we exit early.
    // After this block, TypeScript knows these variables are not null.
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && sessionStorage.getItem('hasSeenHeroAnimation') === 'true') {
          setExperienceState('static');
        }
      },
      { threshold: 0.1 }
    ...
    const animateHero = () => {
        ...
        if (firstPanelContent) {
            firstPanelContent.classList.toggle('is-visible', scrollTop > 1);
        }
  ...
  }, [isClient, experienceState]); 

  // BUG FIX & TYPE-SAFE CLEANUP: This effect handles the state transition.
  useEffect(() => {
    if (experienceState === 'static') {
      // Use optional chaining (`?.`) as a concise and safe way to
      // access properties that might be on a null ref.
